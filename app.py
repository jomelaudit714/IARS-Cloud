...
from iars_excel_conversion import (
    render_invoice_conversion_page,
...
def _apply_v4479_readability_and_library_refinements() -> None:
    """Improve process-step readability, login artwork, and Policies layout."""
    st.markdown(
        """
        <style>
        /* Larger but more compact process stepper text. */
        .edl-stepper {
        ...
        .edl-step:not(:last-child):after {
            margin-left: .12rem !important;
        }

        /* Recover unused top space on Policies & Memoranda. */
        .iars-policies-v4479-marker {display:none !important;}
        .stApp:has(.iars-policies-v4479-marker) .block-container {
            padding-top: 0 !important;
        }
        .stApp:has(.iars-policies-v4479-marker) .edl-topbar {
            margin-top: -25px !important;
            margin-bottom: .30rem !important;
        }
        .stApp:has(.iars-policies-v4479-marker) .edl-section-head {
            margin-top: .04rem !important;
...
def _transition_logo_data_uri() -> str:
    """Return the existing EDL logo as a data URI for transition overlays."""
...
def _apply_v4502_transition_refinements() -> None:
    ...
    st.markdown(
        ...
        /* Header coordinates are controlled only by the V4.5.26 fixed-header
        ...
        .iars-logout-transition-mask {
            ...
            justify-content: center !important;
            background:
                radial-gradient(circle at 50% 42%, rgba(255,255,255,.98) 0 22%, rgba(247,249,252,.98) 55%, rgba(240,244,249,.99) 100%) !important;
        ...
        .iars-logout-transition-fallback {
            ...
            align-items: center !important;
            justify-content: center !important;
            background: linear-gradient(145deg,#0A2C59,#061A36) !important;
            color: #E4AE2F !important;
            border: 3px solid #E4AE2F !important;
            font-size: 1.72rem !important;
        ...
        .st-key-iars_profile_signout_v4_5_03 button {
            ...
            background: linear-gradient(135deg,#061A36,#0A2C59) !important;
...
def _navigate_to_page(nav_label: str) -> None:
    ...
    page_name = nav_label.split(" ", 1)[1] if " " in nav_label else nav_label
    transition_counter = int(st.session_state.get(IARS_PAGE_TRANSITION_COUNTER_KEY, 0) or 0) + 1
    st.session_state[IARS_PAGE_TRANSITION_COUNTER_KEY] = transition_counter
    st.session_state[IARS_PAGE_TRANSITION_KEY] = {
        "page": page_name,
        "token": transition_counter,
    }
...
if isinstance(_pending_page_transition, dict):
    _active_page_transition_token = int(_pending_page_transition.get("token") or 0)
    _render_workspace_transition(
        str(_pending_page_transition.get("page") or "IARS"),
        _active_page_transition_token,
    )
elif _pending_page_transition:
    _active_page_transition_token = int(st.session_state.get(IARS_PAGE_TRANSITION_COUNTER_KEY, 1) or 1)
    _render_workspace_transition(str(_pending_page_transition), _active_page_transition_token)
    ...
    *,
    ...
    with search_col:
        search_text = st.text_input(
            "Search policy title",
            placeholder="Enter all or part of the document title",
    ...
    *,
    ...
    with st.expander("Upload Policy or Memorandum", expanded=False):
        ...
        with left:
            ...
            subject_category = (
                custom_subject_category.strip()
                if subject_category_choice == "Other"
                else subject_category_choice
            )
        with right:
        ...
        if st.button(
            ...
            elif not subject_category.strip():
                st.error("Enter the Subject / Process Category before uploading.")
            else:
...
with st.sidebar:
    ...
    with st.expander("📊 Excel Conversion", expanded=excel_conversion_expanded):
        ...
        sales_invoice_label = "↳ Invoice"
        sales_invoice_selected = selected_page == sales_invoice_label
        st.markdown(
            ...
            '<div style="color:#B8C3D3;font-size:.67rem;font-weight:500;margin-top:.08rem;">LOGP and Invoice active</div></div>',
            unsafe_allow_html=True,
        )
        st.button(
            sales_logp_label,
        ...
        st.button(
            sales_invoice_label,
            key="excel_conversion_nav_invoice",
            use_container_width=True,
            type="primary" if sales_invoice_selected else "secondary",
            on_click=_navigate_to_page,
            args=(sales_invoice_label,),
        )
...
if page_key == "Settings":
    ...
    render_metric_cards(
        [
            {"label": "IARS Version", "value": "4.5.26", "note": "Native Frozen Gantt Grid, Status Colors, and Simplified Initial/Final Report Workflow", "icon": "⚙️", "accent": "#C78B12"},
...
# Signals that the authenticated page has completed rendering. Login and module
